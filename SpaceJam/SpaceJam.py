...
#####################Defense Drones########################################
        ...
        for j in range(fullCycle):
            ...
            # unitVec = mp.BaseballSeams(j, fullCycle)
            # unitVec = mp.Cloud()
...
###################Camera/ Instructions####################################            
    ...
    def setInstruction(self):
        self.inst1 = self.addInstruction(0.95,"[ESC] : Quit")
        self.inst2 = self.addInstruction(0.90,"[Down arrow] : Pitch Forward")
        self.inst3 = self.addInstruction(0.85,"[Up arrow] : Pitch Back")
        self.inst4 = self.addInstruction(0.80,"[Left arrow] : Yaw Left")
        self.inst5 = self.addInstruction(0.75,"[Right arrow] : Yaw Right")
        self.inst6 = self.addInstruction(0.70,"[d] : Roll Left")
        self.inst7 = self.addInstruction(0.65,"[a] : Roll Right")
        self.inst8 = self.addInstruction(0.60,"[Spacebar] : Thrust")
...
###################Camera/ Instructions####################################
    ...
    def setParticels(self):
        ...
        #self.splodeEffect.loadConfig("./Particles/effects/Petesexplode.ptf")
    ...
    def droneDestroy(self, hitID, hitPos):
        ...
        # ptImpact = nodeID.getPos(self.khanShip.origin)
        # print("ptImpact: " + str(hitPos))
...
loadPrcFileData('','win-size 1732, 900')
